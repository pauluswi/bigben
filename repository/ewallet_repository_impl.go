...
func (repository *ewalletRepositoryImpl) Deposit(toAccountNumber int32, amount int) (err error) {
	ctx, cancel := config.NewMySQLContext()
	defer cancel()
	...
	currentBalance, err := repository.Find(toAccountNumber)
	if err != nil {
		return err
	}

	receiverBalance := (currentBalance.Balance + amount)

	query := `UPDATE db_wallet.wallet SET balance = ?, modifieddate = Now() WHERE accountid = ?`

	_, err = tx.ExecContext(ctx, query, receiverBalance, toAccountNumber)
	if err != nil {
		_ = tx.Rollback()
		return err
	}
	...
	insert := `INSERT INTO db_wallet.wallet_trx(accountid, trxtype, dc, trxamount, createdby, createddate, modifiedby, modifieddate) VALUES (?, ?, ?, ?, ?, Now(), ?, Now())`
	_, err = tx.ExecContext(ctx, insert, toAccountNumber, "DEPOSIT", "D", amount, "user", "user")
	if err != nil {
		_ = tx.Rollback()
		return err
	}
...
func (repository *ewalletRepositoryImpl) Withdrawal(fromAccountNumber int32, amount int) (err error) {
	ctx, cancel := config.NewMySQLContext()
	defer cancel()
	...
	insert := `INSERT INTO db_wallet.wallet_trx(accountid, trxtype, dc, trxamount, createdby, createddate, modifiedby, modifieddate) VALUES (?, ?, ?, ?, ?, Now(), ?, Now())`
	_, err = tx.ExecContext(ctx, insert, fromAccountNumber, "WITHDARWAL", "D", amount, "user", "user")
	if err != nil {
		_ = tx.Rollback()
		return err
	}
